...
object Main {
  ...
  @tailrec
  def zadanie3(dniTygodnia: List[String], result:String=""):String = {
    if (dniTygodnia.isEmpty) {
      result
    } else {
      zadanie3(dniTygodnia.tail,
        if(result!=""){
          result + ", " + dniTygodnia.head
        }else{
          dniTygodnia.head
        })
    }
